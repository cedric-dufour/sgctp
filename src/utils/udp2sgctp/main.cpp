...
CSgctpUtil::CSgctpUtil( int _iArgC, char *_ppcArgV[] )
  : CSgctpUtilSkeleton( "udp2sgctp", _iArgC, _ppcArgV )
  , sdInput( -1 )
  , ptAddrinfo_in( NULL )
...
void CSgctpUtil::displayHelp()
{
  // Display full help
  displayUsageHeader();
  cout << "  udp2sgctp [options] [<hostname(in)>]" << endl;
  displaySynopsisHeader();
  cout << "  Dump the SGCTP data received from the given UDP host (default:'localhost')." << endl;
