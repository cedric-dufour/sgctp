...
CSgctpUtil::CSgctpUtil( int _iArgC, char *_ppcArgV[] )
  : CSgctpUtilSkeleton( "sgctp2tcp", _iArgC, _ppcArgV )
...
void CSgctpUtil::displayHelp()
{
  // Display full help
  displayUsageHeader();
  cout << "  sgctp2tcp [options] [<hostname(out)>] [<path(in)>]" << endl;
  displaySynopsisHeader();
  cout << "  Send the SGCTP data of the given file (default:'-', standard input)" << endl;
  cout << "  to the given host (default:localhost), via TCP." << endl;
  ...
  do
  {
    ...
    for(;;)
    {
      ...
      do
      {
        ...
        // ... create socket
        for( __ptAddrinfoActual_out = ptAddrinfo_out;
             __ptAddrinfoActual_out != NULL;
             __ptAddrinfoActual_out = __ptAddrinfoActual_out->ai_next )
        {
          sdOutput = socket( __ptAddrinfoActual_out->ai_family,
        ...
        if( sdOutput < 0 )
        {
          SGCTP_LOG << SGCTP_WARNING << "Failed to create socket (" << sOutputHost << ":" << sOutputPort << ")" << endl;
          break;
        }

        // ... connect socket
        __iReturn = connect( sdOutput,
                             ptAddrinfo_out->ai_addr,
                             ptAddrinfo_out->ai_addrlen );
        if( __iReturn )
        {
          SGCTP_LOG << SGCTP_WARNING << "Failed to connect socket (" << sOutputHost << ":" << sOutputPort << ") @ connect=" << -errno << endl;
          break;
        }

        // Send handshake
        __iReturn = oTransmit_out.sendHandshake( sdOutput );
        if( __iReturn <= 0 )
        {
